...
module.exports = {
    ...
    async execute(member) {
        ...
        const channel = member.guild.channels.cache
            .get(process.env.DISCORD_WELCOME_CHANNEL_ID);
...
async function displayCard(username) {
    ...
    // * If the username has more than 20 characters, splits the welcome message
    // * into two lines.
    if (username.length > 20) {
        ...
        context.fillText(
            `Welcome to The Bunk3r, ${username}!`, canvas.width / 2, 98,
        );
    }

    return new AttachmentBuilder(
        await canvas.encode('png'),
        { name: `card-${username}.png` },
    );
};
